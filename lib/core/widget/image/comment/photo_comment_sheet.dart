...
import 'package:ddara/core/widget/image/comment/sheet_expand_mixin.dart';
...
class PhotoCommentSheetState extends State<PhotoCommentSheet>
    with WidgetsBindingObserver, SheetExpandMixin {
  ...
  /// 댓글 목록 상태와 서버 연동. 화면은 이 컨트롤러를 구독해 그리기만 한다.
  late final CommentListController _list = CommentListController(
    ...
    onListReloaded: _scrollToNewest,
  );

  /// 댓글 목록 스크롤. 본문 드래그를 시트 확장과 나눠 갖는 전용 컨트롤러다.
  /// (댓글 등록 시 최신 댓글 쪽으로 이동하는 데도 쓴다)
  late final SheetScrollController _commentScrollController =
      SheetScrollController(this);
  ...
  @override
  FocusNode get inputFocusNode => _commentFocusNode;
  ...
  bool get hasPendingComments => _list.hasPending;

  /// 댓글 목록을 조회해 서버 목록으로 갱신한다. 시트를 열 때마다 호출되어,
  /// 닫았다 다시 열면 최신 목록으로 재조회된다.
  Future<void> reload() => _list.reload();
  ...
  ///
  /// 등록은 낙관적 업데이트라 입력값을 즉시 비운다. 연타로 같은 내용이 두 번
  /// 나가지 않고, 실패한 댓글은 목록에 남아 거기서 재전송한다.
  /// 수정은 실패 시 입력값·키보드를 유지해 바로 재시도할 수 있게 한다.
  Future<void> _submitComment(String text) async {
    ...
    if (editing != null) {
      if (await _list.applyEdit(editing, content) && mounted) {
        _exitEditComment();
      }
      return;
    }
    ...
    final justNowLabel = AppLocalizations.of(context).timeAgoJustNow;
    _commentController.clear();
    await _list.submit(
      content: content,
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedPositioned(
      duration: isDragging ? Duration.zero : PhotoCommentSheet.duration,
          ...
          child: Column(
            children: [
              ...
              AnimatedOpacity(
                ...
                opacity: _bodyScrolled && _list.comments.isNotEmpty ? 1 : 0,
  ...
  Widget _header() {
    return GestureDetector(
      ...
      onVerticalDragStart: (_) => onHeaderDragStart(),
      onVerticalDragUpdate: (details) => applyExpandDrag(details.primaryDelta!),
      onVerticalDragEnd: onHeaderDragEnd,
      onVerticalDragCancel: cancelHeaderDrag,
      child: Column(
        children: [
          // 상단 그랩 핸들.
          Container(
  ...
  Widget _body(AppLocalizations l10n) {
    ...
    if (_list.isLoading && comments.isEmpty) {
    ...
    return LazyRevealList(
      ...
      builder: (context, visibleComments) => ListView.builder(
        ...
        itemBuilder: (context, index) => CommentItem(
          ...
          onDelete: _list.delete,
          onReport: _list.report,
          onBlock: _list.block,
          onRetry: _list.retry,
          onDiscard: _list.discard,
        ),
      ),
    );
  }
